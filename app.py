...
def image_sketch(decode_array_to_img):
    ...
    sharping_img = cv2.bitwise_not(converted_gray_img)
    bluring_img = cv2.GaussianBlur(sharping_img,(111,111), 0)
    sharping_blurred_img = cv2.bitwise_not(bluring_img)
    sketch_img = cv2.divide(converted_gray_img, sharping_blurred_img, scale=256.0)
...
def water_color_effect(decode_array_to_img):
    ...
    status, output_image = cv2.imencode('.PNG', water_effect_img)
     
    return output_image     

def invert(decode_array_to_img):
    invert_effect_img = cv2.bitwise_not(decode_array_to_img)
    status, output_image = cv2.imencode('.PNG', invert_effect_img)
...
def HDR(decode_array_to_img):
    ...
    status, output_image  = cv2.imencode('.PNG', hdr_effect_img)
